...
# Global variables 
map = {"A":13, "K":12, "Q":11, "J":10, "T":9, "9":8, "8":7, "7":6, "6":5, "5":4, "4":3, "3":2, "2":1}
...
with open("input.txt") as file:
    ...
    while nextLine:
        ...
        hands[-1].append([k for k in (sorted([hands[-1][0].count(k) for k in dict.fromkeys(sorted(hands[-1][0]))], reverse=True))])  
        hands[-1].append(handScore(hands[-1]))
        
        nextLine = file.readline()

hands = sorted(hands, key=operator.itemgetter(4))
result_1 = sum([hands[x][2]*(x+1) for x in list(range(len(hands)))])
